<?php 
    ...
    $sql= "SELECT quizes.quizID, quizes.titulo, usuarios.nome as criador, categorias.nome, quizes.dificuldade , quizes.expira FROM quizes INNER JOIN categorias on quizes.categoria=categoriasID INNER JOIN usuarios on quizes.criador=usuarios.usuarioID;";
    ...
    <?php if (isset($_POST['apagado'])){
        if ($_POST['apagado']==1):?>
            <div class='alert alert-info  m-0' role='alert'>
                Quiz apagado com sucesso! 
            </div>
        <?php else: ?>
            <div class='alert alert-danger  m-0' role='alert'>
                Erro ao apagar o quiz!
            </div>

        <?php endif; ?>
    <?php };?>
    <h3 class='display-6 text-dark'>QUIZES CADASTRADOS</h3>
        <table class='table table-hover table-sm '>
            ...
            <tbody>
                ...
                while ($linha=$quizes->fetch_assoc()):?>
                        ...
                        <td><?php echo $linha['expira'];?></td>
                        <td>
                            <form method='POST' action='removequiz.php' style='display: inline'>
                                <input type='hidden' value='<?php echo $linha['quizID']?>' name='id'>
                                <input type='submit' value='Apagar' name='removequiz' class='btn btn-danger'>
                            </form>
                        </td>
                        </tr>
                <?php endwhile;?>
