<?php 
    function console_log( $data ){
      ...
      $sql= 'select quizes.criador as "ID do criador", usuarios.nome as "Nome do criador", quizes.quizID as "ID do Quiz",quizes.titulo as "Nome do quiz", quizes.dificuldade as "Dificuldade", quizes.categoria as "ID da Categoria", categorias.nome as "Categoria", quizes.expira as "Data de Expiraçao" from usuarios inner join quizes on quizes.criador = usuarios.usuarioID inner join categorias on categorias.categoriasID = quizes.categoria order by categorias.nome, quizes.titulo
      ';
      // $sql="SELECT quizes.criador, usuarios.nome, quizes.quizID,quizes.titulo, quizes.dificuldade, quizes.categoria, categorias.nome from usuarios inner join quizes on quizes.criador = usuarios.usuarioID inner join categorias on categorias.categoriasID = quizes.categoria order by categorias.nome, quizes.titulo
      // ";
...
<html lang="pt-br">
  ...
  <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    ...
    <section class="hero-wrap hero-wrap-2" style="background-image: url('https://ak3.picdn.net/shutterstock/videos/3367433/thumb/4.jpg');">
      <div class="overlay"></div>
      <div class="container">
        <div class="row no-gutters slider-text align-items-center justify-content-center">
          <div class="col-md-9 ftco-animate text-center">
            <h1 class="mb-2 bread">NOSSOS QUIZES</h1>
    ...
    <div class="row">
    <?php $col=0;
    while ($linha = $results->fetch_assoc()) :console_log($linha)?>
      <div class="col-sm-6 col-lg-4">
        <div class="card m-3"  >
          <div class="card-body">
            <h5 class="card-title"><?php echo $linha['Nome do quiz']?></h5>
              <p class="card-text">
                Criado por: <strong><?php echo $linha['Nome do criador']?></strong><br>
                Dificuldade: <strong><?php echo $dificuldades[$linha['Dificuldade']] ?></strong><br>
                Categoria: <strong><?php echo $linha['Categoria']?> </strong><br>
                Data de Expiracao: <strong><?php echo $linha['Data de Expiraçao']?> </strong><br>
                Situação: <strong><?php if(strtotime($date)<=strtotime($linha['Data de Expiraçao'])){echo 'Ativo';}else{echo 'Inativo';}?> </strong>
              </p>
              <form method='POST' action='playquiz.php' style='display: inline'>
                <input type='hidden' name='id' value='<?php echo $linha['ID do Quiz']?>'>
                <input type='submit' class='btn btn-primary
                <?php if (strtotime($date)>strtotime($linha['Data de Expiraçao'])):?>
                  "disabled"
                <?php endif;?>' value='Jogar' name='jogar'>
              </form>
              <?php if($_SESSION['admin']==1):?>
              <form method='POST' action='lista_de_perguntas.php' style='display: inline'>
                <input type='hidden' name='id' value='<?php echo $linha['ID do Quiz']?>'>
                <input type='submit' class='btn btn-outline-primary
                <?php if (strtotime($date)>strtotime($linha['Data de Expiraçao'])):?>
                  "disabled"
                <?php endif;?>' value='Editar' name='editar'>
              </form>
              <?php endif;?>
